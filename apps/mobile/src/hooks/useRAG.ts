...
const CHUNK_SIZE = 15 * 1024 // 15 KB of base64 per message
...
export const useRAG = () => {
  ...
  useEffect(() => {
    onRagMessageRef.current = (msg: any) => {
      if (msg.type === 'rag_list_response') {
        ...
        setDocuments((prev) =>
          prev.filter((d) => d.doc_id !== msg.docId),
        )
  ...
  // Reset in-flight state if the connection drops
  useEffect(() => {
    if (authState !== 'ready') {
      setUploading(false)
      setLoading(false)
