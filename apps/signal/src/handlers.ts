...
async function verifySignature(
  ...
  try {
    ...
    const pubKeyField = (msg.pubKey ?? msg.from) as
      | string
      | undefined
    ...
    const sigBytes = base64ToBytes(sig as string)
    const pubKeyBytes = base64ToBytes(pubKeyField)

    console.log(
      `[signal:verify${label}] payload str : ${payloadStr}`,
    )
    console.log(
      `[signal:verify${label}] payload hex : ${Buffer.from(payloadBytes).toString('hex')}`,
    )
    console.log(
      `[signal:verify${label}] sig         : ${sig}`,
    )
    console.log(
      `[signal:verify${label}] pubKey      : ${pubKeyField}`,
    )

    const result = ed.verify(
      sigBytes,
      payloadBytes,
      pubKeyBytes,
    )
    console.log(
      `[signal:verify${label}] result      : ${result}`,
    )
    return result
  } catch (e) {
    console.error(
      `[signal:verify${label}] exception   : ${e}`,
    )
    return false
  }
}
...
export function createMessageHandler(
  ...
  return async function handle(
    ...
    if (type === 'join') {
      ...
      const valid = await verifySignature(msg, ':join')
      if (!valid) {
        return error(
          'INVALID_SIGNATURE',
      ...
      // Check if clientId already in room - deduplicate
      ...
      const peers = existingRoom
        ? Array.from(existingRoom.values())
        : []
      ...
      rooms.broadcastToRoom(
        roomId,
        { type: 'peer_joined', clientId, pubKey },
        clientId,
      )

      console.log(
        `[signal] ${clientId} joined room ${roomId}`,
      )
      return
    }
    ...
    if (['offer', 'answer', 'ice'].includes(type)) {
      ...
      if (
        !ts ||
        Math.abs(Date.now() - ts) > 5 * 60 * 1000
      ) {
        return error(
          'STALE_TIMESTAMP',
          'Message timestamp too old or in future',
      ...
      // For offer/answer/ice the sig is from the sender, pubKey used for verification is sender's
      const msgWithSenderPubKey = {
        ...msg,
      ...
      const target = rooms.getClient(
        sender.roomId,
        toClientId,
      )
      if (
        !target ||
        target.ws.readyState !== target.ws.OPEN
      ) {
        return error(
          'PEER_NOT_FOUND',
          `Peer ${toClientId} not found or not connected`,
        )
      }
